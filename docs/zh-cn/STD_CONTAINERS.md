...
本文侧重 C++ 容器。`StdArray` / `StdVector` / `StdMap` / `StdOrderedMap` / `StdList` / `StdDict` 统一称为“类型注解”，完整设计与不同存储边界见 [类型注解统一设计](TYPE_ANNOTATIONS.md)。普通 PHP 数组模型的 list/dict 见 [强类型 PHP 数组](TYPED_ARRAYS.md)。
